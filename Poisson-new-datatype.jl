...
function plot_error_contourf(u_numeric, u_exact, Nx, Ny, h, title_str)
    error_grid = reshape(abs.(u_numeric .- u_exact), Nx, Ny)'
    x = 0:h:(Nx - 1)*h
    y = 0:h:(Ny - 1)*h
    contourf(x, y, error_grid;
             xlabel="x", ylabel="y", title=title_str,
             color=:plasma, aspect_ratio=1)
end

plot_error_contourf(u_jacobi_new, u_exact, Nx, Ny, h, "Error (Jacobi vs Exact)")
plot_error_contourf(u_gauss_seidel_new, u_exact, Nx, Ny, h, "Error (Gauss-Seidel vs Exact)")
